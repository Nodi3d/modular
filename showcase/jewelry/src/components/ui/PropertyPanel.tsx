...
export const PropertyPanel: React.FC = () => {
  const { nodes, updateNodeProperty, nodeIds, currentType } =
    useModularWorkerStore();
  ...
  const updateParameteStore = useCallback(
    (label: string, value: number) => {
      if (currentType === "braid") {
        ...
        if (label === "waveCount")
          setBraidParameters({ ...braidParameters, waveCount: value });
        if (label === "waveSize")
          setBraidParameters({ ...braidParameters, waveSize: value });
        if (label === "width")
          setBraidParameters({ ...braidParameters, width: value });
        if (label === "thickness")
          setBraidParameters({ ...braidParameters, thickness: value });
        if (label === "ringCount")
          setBraidParameters({ ...braidParameters, ringCount: value });
      } else if (currentType === "bypass") {
        if (label === "innerDiameter")
          setBypassParameters({ ...bypassParameters, innerDiameter: value });
        if (label === "startWidth")
          setBypassParameters({ ...bypassParameters, startWidth: value });
        if (label === "endWidth")
          setBypassParameters({ ...bypassParameters, endWidth: value });
        if (label === "length")
          setBypassParameters({ ...bypassParameters, length: value });
        if (label === "gap")
          setBypassParameters({ ...bypassParameters, gap: value });
        if (label === "thickness")
          setBypassParameters({ ...bypassParameters, thickness: value });
      } else if (currentType === "twist") {
        if (label === "innerDiameter")
          setTwistParameters({ ...twistParameters, innerDiameter: value });
        if (label === "ticknessBottom")
          setTwistParameters({ ...twistParameters, ticknessBottom: value });
        if (label === "ticknessTop")
          setTwistParameters({ ...twistParameters, ticknessTop: value });
        if (label === "twist")
          setTwistParameters({ ...twistParameters, twist: value });
        if (label === "polygon")
          setTwistParameters({ ...twistParameters, polygon: value });
      }
    },
    [
      currentType,
  ...
  const getParameterValue = useCallback(
    (label: string) => {
      if (currentType === "braid") {
        ...
        if (label === "ticknessBottom") return twistParameters.ticknessBottom;
        if (label === "ticknessTop") return twistParameters.ticknessTop;
          ...
          if (
            ...
            return (
              ...
              >
                ...
                />
                <span className="w-9 text-right block">
                  {v !== undefined ? v : value.content}
                </span>
              </div>
            );
          }
  ...
  return (
    <div
      className="absolute bottom-24 inset-x-0 z-10 font-serif"
      ref={panelRef}
    >
      <motion.div
        layout
          ...
          <>
            ...
            <p
              className={`cursor-pointer text-center p-2 flex flex-col items-center justify-center${
                currentMenu == 0
                  ? "text-content-h-a"
                  : "text-content-l-a hover:text-content-h-a"
              } transition-all`}
              onClick={() => setCurrentMenu(0)}
            >
              <span className="text-xs">
                {size.locale.toUpperCase() + " size"}
              </span>
